<?php
	...
	if ( !isset($cs_node->cs_event_view) || empty($cs_node->cs_event_view) ) { $cs_node->cs_event_view = 'eventlisting'; }
	  $meta_compare = '';
        ...
        if ( isset($_GET['event_category']) ) {$event_category = $_GET['event_category'];}
        else {
            if(isset($row_cat->slug)){
            $event_category = $row_cat->slug;
            }
        }
		$cs_counter_events = 0;
		if ( empty($_GET['page_id_all']) ) $_GET['page_id_all'] = 1;
			...
			if(isset($event_category) && $event_category <> '' && $event_category <> '0'){
				$event_category_array = array('event-category' => "$event_category");
		...
		?>  
                ...
                <ul class="sortby">
                         ...
                         <?php
                            ...
                            ?>
                            <li class="<?php if(($cs_node->cs_event_category==$event_category)){echo 'bgcolr';}?>">
                            	<a href="?<?php echo $qrystr."&event_category=".$row_cat->slug?>"><?php _e("All",'Statfort');?></a>
                            </li>
                            <?php
                            }else{
                            ...
                            ?>
                                <li <?php if($category->slug==$event_category){echo 'class="bgcolr"';}?>><a href="?<?php echo $qrystr."&event_category=".$category->slug?>"><?php echo $category->cat_name?></a>
                                </li>
                       <?php }?>
                    </ul>
                <!-- Sortby End -->
    ...
    <div class="event <?php echo $cs_node->cs_event_view;?>">
        ...
        $qrystr = '';
          ...
          if ( $cs_node->cs_event_pagination == "Show Pagination" && $count_post > $cs_node->cs_event_per_page && $cs_node->cs_event_per_page > 0 && $cs_node->cs_event_filterables != "Yes" ) {
				if ( isset($_GET['page_id']) ) $qrystr = "&page_id=".$_GET['page_id'];
